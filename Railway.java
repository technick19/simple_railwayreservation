    ...
    static void bookTicket(){
        ...
        for (Train train : trains) {
            if (train.TrainName.equals(tName)) {
                ...
                int availableTicket = availTic(train, nop, st, en);
                if (nop != availableTicket) {
                    if (availableTicket>0) System.out.println(availableTicket +" : Seats are Available "+
                            (nop-availableTicket)+" : Seat Will be in WaitingList!");
                ...
                if (sc.nextInt() == 1) {
                    allotTicket(train, nop, st, en, availableTicket);
                }
                break;
            }
        }
